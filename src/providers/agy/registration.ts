...
import {
  agyWorkspaceRegistration,
  getAgyWorkspaceServices,
} from './app/AgyWorkspaceServices';
...
export const agyProviderRegistration: ProviderModule = {
  ...
  createExecutionBackend: (plugin) => new AgyExecutionBackend(
    plugin,
    getAgyWorkspaceServices(),
  ),
  resolveTitleGenerationModel: (plugin) => {
    ...
    const titleModel = typeof settings.titleGenerationModel === 'string'
      ...
      : '';
    return agyChatUIConfig.ownsModel(titleModel, settings) ? titleModel : undefined;
  },
